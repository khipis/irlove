/**
 * Fallback for animal quest text when the LLM module is not loaded (e.g. file:// or no network).
 * Load this before the module; the module will overwrite window.generateAnimalQuest when it loads.
 * No Node, no build – works from file:// and GitHub Pages.
 */
(function () {
  'use strict';

  var POLISH_TEMPLATES = [
  ...
  function getPolishQuest(animalName) {
    var template = POLISH_TEMPLATES[Math.floor(Math.random() * POLISH_TEMPLATES.length)];
    return (animalName ? animalName + ' mówi: ' : '') + template;
  }

  function getEnglishFallback(animalName) {
    var template = ENGLISH_FALLBACKS[Math.floor(Math.random() * ENGLISH_FALLBACKS.length)];
    return (animalName ? animalName + ' says: ' : '') + template;
  }

  function generateAnimalQuest(animalName, lang) {
    var name = animalName || (lang === 'pl' ? 'Zwierzątko' : 'Animal');
    var text = lang === 'pl' ? getPolishQuest(name) : getEnglishFallback(name);
